...
try {
    ...
    $addHoa = 'INSERT INTO hoa_list(hoa_number,head_of_dept,budget_year,estb_scheme,major_head_num,major_head,sub_major_head_num,sub_major_head,minor_head_num,minor_head,group_sub_head_num,group_sub_head,sub_head_num,sub_head,detailed_head_num,detailed_head,sub_detailed_head_num,sub_detailed_head,charged_voted) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
    $stmt = $pdo->prepare($addHoa);
    $stmt->execute([$hoaNumber, $headOfDept, $budgetYear, $estbScheme, $majorHeadNum, $majorHead, $subMajorHeadNum, $subMajorHead, $minorHeadNum, $minorHead, $groupSubHeadNum, $groupSubHead, $subHeadNum, $subHead, $detailedHeadNum, $detailedHead, $subDetailedHeadNum, $subDetailedHead, $charVoted]);
    $response['status'] = true;
    $response['message'] = 'HOA List Added Successfully.';
    echo json_encode($response);
    exit;
